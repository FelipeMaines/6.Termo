export class Jogo{
    palavras = [ "ácido", "adiar", "ímpar", "algar", "amado", "amigo", "anexo", "anuir", "aonde", "apelo",
    ...
    SortearPalavra(): string{
        let valorAleatorio: number = Math.floor(Math.random() * this.palavras.length);

        this.palavraSecreta = this.palavras[valorAleatorio];
        return this.palavraSecreta;
    }

    VerificarAcerto(palavraChutada: string) : Array<number>{
        this.linha++;
    ...
    JogadorGanhou(palavra: string) : boolean{
